...
def main():
    ...
    for bean in beans:
        bean.position.xy = (
            random.randrange(0, DISPLAY.get_width() - bean.sprite.get_width()),
    ...
    while splashScreenTimer < 100:
        ...
        startMessage = font_small.render("IDP Batch-6", True, (171, 145, 123))
        DISPLAY.blit(
            startMessage,
            (
    ...
    while titleScreen:
        ...
        DISPLAY.blit(
            logo,
            (
                DISPLAY.get_width() / 2 - logo.get_width() / 2,
                DISPLAY.get_height() / 2
                - logo.get_height() / 2
                + math.sin(time.time() * 5) * 5
                - 25,
            ),
        )
        DISPLAY.blit(
            retry_button, (DISPLAY.get_width() / 2 - retry_button.get_width() / 2, 288)
        )
        startMessage = font_small.render("START", True, (0, 0, 0))
        DISPLAY.blit(
            startMessage, (DISPLAY.get_width() / 2 - startMessage.get_width() / 2, 292)
        )
    ...
    while True:
        ...
        DISPLAY.blit(
            ...
            (
                ...
                camOffset
                + round((player.position.y - startingHeight) / DISPLAY.get_height())
                * DISPLAY.get_height()
        ...
        DISPLAY.blit(
            pygame.transform.rotate(
                player.currentSprite, clamp(player.velocity.y, -10, 5) * rotOffset
            ),
        ...
        for button in buttons:
            ...
            DISPLAY.blit(
                button.typeIndicatorSprite, (202 + (buttons.index(button) * 125), 377)
            )
                ...
                if beanCount >= button.price:
                    ...
                    if buttons.index(button) == 2:
                        ...
                        for i in range(beanMultiplier):
                            beans.append(Bean())
                            beans[-1].position.xy = random.randrange(
                                0, DISPLAY.get_width() - bean.sprite.get_width()
                            ), player.position.y - DISPLAY.get_height() - random.randrange(
                                0, 200
                            )
